...
def index (request):
    ...
    if 'on' in request.GET:
        ...
        if FC not in GR or SC not in GR:           
            ...
            return render(request,'pages/index.html',{'f':FC ,'s':SC ,'H':HEURISTIC,'C':cost,'Q':Queue})
    if 'tos' in request.GET:
        ...
        if 'Connctio' in request.GET:
            ...
            if 'Heu' in request.GET:
                ...
                if New not in GR:
                    GR[New]={conn:int(Heu)}
        return render(request,'pages/index.html',{'NEW':New ,'Conn':conn ,'He':Heu,'C':cost,'G':GR,'a':S_L})
    else:
